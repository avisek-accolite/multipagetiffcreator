...
public class OtsuThreshold {
	...
	public double getOtsuThreshold(BufferedImage image) {
		int [][] grayScale = new int[image.getWidth()][image.getHeight()];
		
		for(int i=0; i<image.getWidth(); i++) {
			for(int j=0; j<image.getHeight(); j++) {
				grayScale[i][j] = image.getRGB(i, j) & 0xFF;
			}
		}
    ...
    //Probability = no of pixel of this color/total pixels
    ...
    //Wo is the cumulative probability
